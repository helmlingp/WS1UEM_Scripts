...
function Invoke-GetRoles {

  $url = "$server/API/system/roles"
  $header = @{'aw-tenant-code' = $APIKey;'Authorization' = $cred;'accept' = 'application/json;version=2';'Content-Type' = 'application/json'}
  try {
    $allroles = Invoke-RestMethod -Method Get -Uri $url.ToString() -Headers $header
  }
  catch {
    throw "Server Authentication or Server Connection Failure $($_.Exception.Message)`n`n`tExiting"
  }
...
function Invoke-GetAdminGroups {
  param(
    [Parameter(Mandatory=$true)][string]$Domain,
    [Parameter(Mandatory=$true)][string]$groupuuid
  )
  #MIGHT BE DEV API AND NOT AVAILABLE TO PROD
  $url = "$server/api/system/admin-groups/search"
  $header = @{'aw-tenant-code' = $APIKey;'Authorization' = $cred;'accept' = 'application/json;version=2';'Content-Type' = 'application/json'}
  $body
  try {
    $Response = Invoke-RestMethod -Method Get -Uri $url.ToString() -Headers $header
    $AdminGroups = $Response.results
  }
  catch {
    throw "Server Authentication or Server Connection Failure $($_.Exception.Message)`n`n`tExiting"
  }
...
function Invoke-AddADGroup {
  param(
    [Parameter(Mandatory=$true)][string]$UserGroup
  )
  #MIGHT BE DEV API AND NOT AVAILABLE TO PROD
  $url = "$server/api/system/admin-groups/create"
...
function Invoke-AddRole {
  ...
  #MIGHT BE DEV API AND NOT AVAILABLE TO PROD
  $url = "$server/api/system/groups/$ADGroup/admins/$AdminGroup/roles/$Role"
  #if role is assigned

  $header = @{'aw-tenant-code' = $APIKey;'Authorization' = $cred;'accept' = 'application/json;version=2';'Content-Type' = 'application/json'}
  try {
    $addroles = Invoke-RestMethod -Method Put -Uri $url.ToString() -Headers $header
  }
  catch {
    throw "Server Authentication or Server Connection Failure $($_.Exception.Message)`n`n`tExiting"
  }
...
<#
  CSV Example
  UserGroup,OGName,Role,ADDomain
#> 
  ...
  #Get list of Admin Groups already added to WS1 UEM
  $ADGroups = Invoke-GetDomainGroups -groupuuid $groupuuid

  #Get list of Roles
  $allroles = Invoke-GetRoles
  ...
  foreach($item in $csv){
    ...
    if($ADGroupsearch){
      ...
      write-host $UserGroupid
    } else {
      Write-Log2 -Path "$logLocation" -Message "AD Group $UserGroup does not exist, please add" -Level Error
      #Invoke-AddADGroup -UserGroup $UserGroup
    }
    
<#     $Rolessearch = $allroles | Where-Object {($_.name) -eq $Role}
    if($Rolessearch){
      Write-Log2 -Path "$LogLocation" -Message "WS1 UEM Role $Role exists"
      $addrole = $false
    } else {
      Write-Log2 -Path "$LogLocation" -Message "WS1 UEM Role $Role does not exist"
      $addrole = $true

    } #>

    #Add SmartGroup with UserGroup as member
    $newSG = "SG_$OGName"
    $SGsearch = $allSmartGroups | Where-Object {($_.Name) -eq "$newSG"}
    if($SGsearch){
      Write-Log2 -Path "$LogLocation" -Message "Smart Group $newSG exists"
      $addSG = $false
    } else {
      Write-Log2 -Path "$LogLocation" -Message "Smart Group $newSG does not exist"
      $addSG = $true
      Invoke-CreateSG -newSG $newSG -managedbyOGid $managedbyOGid -UserGroup $UserGroup -UserGroupid $UserGroupid
      if($createSGresponse){
